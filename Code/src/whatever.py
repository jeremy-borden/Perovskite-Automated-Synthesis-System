...
class ProcedureFrame(ctk.CTkScrollableFrame):
    def __init__(self, master, procedure_load_save: Procedure):
        ...
        self.titleLabel = ctk.CTkLabel(self, text="Procedure", justify="left",
                                       font=("Arial", 14, "bold")
                                       ).grid(row=0, column=0)

        self.btn_frame = ctk.CTkFrame(
            self, height=100, border_color="#1f6aa5", border_width=2)
        self.btn_frame.grid(row=1, column=0, sticky="nwe", padx=10, pady=10)

        self.add_step_button = ctk.CTkButton(
            self.btn_frame, text="Add Step", width=20, height=20, command=self.onAddStep)
        self.add_step_button.grid(
            row=1, column=0, sticky="nw", padx=10, pady=10)

        self.step_dropdown = ctk.CTkOptionMenu(
            self.btn_frame, width=50, height=20, values=["Heat", "Spin", "Apply", "Mix"])
        self.step_dropdown.grid(row=1, column=1, sticky="nw", padx=10, pady=10)

        self.delete_step_button = ctk.CTkButton(
            self.btn_frame, text="Delete Step", width=20, height=20, command=self.onRemoveStep)
        self.delete_step_button.grid(
            row=1, column=2, sticky="nw", padx=10, pady=10)

        self.save_button = ctk.CTkButton(
            self.btn_frame, text="Save", width=20, height=20, command=self.saveProcedure)
        self.save_button.grid(row=1, column=3, sticky="nw", padx=10, pady=10)

        self.load_button = ctk.CTkButton(
            self.btn_frame, text="Load", width=20, height=20, command=self.loadProcedure)
    ...
    def saveProcedure(self):
        ...
        path = filedialog.asksaveasfilename(
            initialdir="src/", filetypes=(("Yaml files", "*.yml"),))
...
class StepFrame(ctk.CTkFrame):
    def __init__(self, master, title):
        super().__init__(master, border_color="#1f6aa5",
                         border_width=2, width=400, height=100)
        self.bind("<Button-1>", command=self.onFocusIn)

        pad = 5
        self.titleLabel = ctk.CTkLabel(self, text=title, justify="left", font=(
            "Arial", 14, "bold")).grid(row=0, column=0, sticky="nw", padx=pad, pady=pad)
...
class EntryLabel(ctk.CTkFrame):
    def __init__(self, master, label_text):
        ...
        self.entry_label = ctk.CTkLabel(
            self, text=label_text, justify="left", width=LABEL_WIDTH, anchor="w")
...
class DropdownLabel(ctk.CTkFrame):
    def __init__(self, master, label_text, options):
        ...
        self.dropdown_label.grid(
            row=0, column=0, sticky="nw", padx=pad, pady=pad)
...
class ApplyStep(StepFrame):
    def __init__(self, master):
        ...
        self.application_duration.grid(
            row=2, column=0, sticky="nw", padx=pad, pady=pad)
...
class MixStep(StepFrame):
    def __init__(self, master):
        ...
        self.destination_vial = DropdownLabel(
            self, "Destination Vial: ", self.options)
        self.destination_vial.grid(
            row=1, column=0, sticky="nw", padx=pad, pady=pad)

        self.btn_frame = ctk.CTkFrame(self)
        self.btn_frame.grid(row=2, column=0, sticky="nw", padx=pad, pady=pad)

        self.newVialButton = ctk.CTkButton(
            self.btn_frame, text="New Vial", width=20, height=20, command=self.newVial).grid(row=2, column=0, sticky="nw")
        self.delete_vial_button = ctk.CTkButton(
            self.btn_frame, text="Delete Vial", width=20, height=20, command=self.deleteLastVial).grid(row=2, column=1, sticky="nw")

        self.newVial()
        self.newVial()

    def newVial(self):
        if (len(self.vial_frames) == len(self.options)):
            return

        frame = ctk.CTkFrame(self, width=400, height=100,
                             border_color="#1f6aa5", border_width=2)
        frame.grid(row=3 + len(self.vial_frames),
                   column=0, sticky="nw", padx=2, pady=2)

        dropdown = DropdownLabel(frame, "Source Vial: ", [
